...
from LOSS import MTLUW, MTLUWR, MTLDWA
...
def main():
    ...
    loop = trange(epochs)
    for epoch in loop:
        train_losses, r_loss, w, train_acc = train(model, train_loader, optimizer, criterion)
    ...
    plt.savefig("./logs_dwa.jpg")
    plt.figure()
    plt.plot(ws.reshape([epochs, 2]))
    plt.legend(["w1", "w2"])
    plt.savefig("./ws_dwa.jpg")
